...
# Create stacked bars
p1 = ax.barh(y_pos, java, label='Java', color=java_color)
p2 = ax.barh(y_pos, python, left=java, label='Python', color=python_color)
cpp_left = [j + p for j, p in zip(java, python)]
p3 = ax.barh(y_pos, cpp, left=cpp_left, label='C++', color=cpp_color)

# Customize
ax.set_yticks(y_pos)
ax.set_yticklabels(models, fontsize=16)
ax.set_ylim(-0.5, len(models) - 0.5)  # Add this line
