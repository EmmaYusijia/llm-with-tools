...
def components(comp1, comp2, comp3, comp4, comp5, comp6, ax, width, x, color1, color2, color3):
    ...
    git_show = np.array([0, 0, 6, 0, 0])
    edit = np.array([0, 0, 3, 0, 0])
    cat = np.array([0, 0, 2, 0, 0])
    python3 = np.array([0, 0, 2, 0, 0])
    pythonm = np.array([0, 0, 1, 0, 0])
    git_log = np.array([0, 0, 4, 0, 0])
    lso = np.array([0, 0, 0, 0, 3])
    pwd = np.array([0, 0, 0, 0, 2])

    arr = [git_show, edit, cat, python3, pythonm, git_log, lso, pwd]
    labels     = ["git show", "edit", "cat", "python3 -c", "python -m pytest", "git log", "ls", "pwd"]
    colors     = ["silver", "rosybrown", "lightcoral", "sandybrown", "goldenrod", "darkseagreen", "powderblue", "plum"]
    ...
    ax.bar(w_x, ls, width, bottom = comp4 + comp5 + comp6, color="#8fc3c3")
    

lst1, lst2, lst3, lst4, lst5, lst6 = get_tool_types_from_xlsx("python_tool_types.xlsx")
bar_plot("python", lst1, lst2, lst3, lst4, lst5, lst6)